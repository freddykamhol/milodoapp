...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!within7Days) return NextResponse.json({ ok: false, error: "not_acute_window" }, { status: 409 });

  const result = await triggerAppointmentInquiry(appointmentId, "URGENT_REQUESTS");
  if (!result.anyOk) {
